...
Tras el cierre de los sub-paquetes P2.1 (commit `9d2662c`, vendor real activo en TEST) y P2.2 (commit `3e97839`, delegación de decisión AMBER/RED al `summary.action` del workflow Sightengine), y tras la Fase 2 de refactor del lenguaje del producto (2026-06-27, sin commit propio: refactor R1+R2+R4 + anotación G3+G4+R5), quedan abiertas diez deudas estratégicas que NO viven en bitácora porque son **transversales** al frente y bloquean trabajos no técnicos del proyecto (compliance Segpay, comunicaciones, refactor de policies, activación entornal, tracking de aceptación, fuente del Model Contract). Las deudas tácticas (latencia métricas, pool S3 separado, panel preview imagen, etc.) sí viven en la bitácora del 2026-06-27.
...
**Naturaleza del bloque**: estas 10 deudas son catálogo, no orden impuesto. El operador secuencia caso por caso. Las deudas tácticas del frente moderación (métricas de latencia, panel admin preview de evidencia, separación de pools S3, centralización de @Async, saneamiento de WARN del mapper, runbook permanente del setup entornal) viven en la entrada de bitácora del 2026-06-27 (`project-log.md`), no aquí.
