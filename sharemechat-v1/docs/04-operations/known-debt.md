...
**#D-19 — Cooldown antifraude automático por chargeback**. `../01-business/affiliate-program.md` §4 fija cooldown >3% chargeback sostenido 3 meses como criterio de suspensión. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D9 sostiene que en la fase inicial el operador suspende manualmente vía endpoint admin (`POST /api/admin/affiliate/deactivate`). La automatización con detección + suspensión + revisión manual queda como sub-fase posterior cuando el volumen justifique. **Prioridad**: media pre-go-live PROD.

**#D-20 — Webhook PSP para chargebacks/refunds automáticos**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D8 materializa el reverso de comisión vía endpoint admin manual (`POST /api/admin/affiliate/chargeback`). En la fase actual (soft launch cripto vía Paxum/NOWPayments, ADR-047) no hay ventana de chargeback tarjeta y el flujo manual basta. Cuando el PSP tarjeta esté integrado con webhook de chargebacks, el motor de comisiones lo consume automáticamente. **Prioridad**: alta cuando se integre PSP tarjeta.

**#D-21 — Panel admin de afiliaciones**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D13 subpasada 8 lo declara opcional en la fase inicial. Sin él, el operador consulta por SQL directo el estado de las comisiones y los payouts filtrando por `payout_requests.payout_type='AFFILIATE'`. Panel React con listado, comisiones agregadas, chargebacks, alertas de cooldown se abre como frente propio cuando el volumen lo justifique. **Prioridad**: baja hasta que haya ≥5 afiliadas activas.

**#D-22 — Detección de auto-referral**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` deudas diferidas: una modelo comparte su URL, se auto-registra como cliente con otra cuenta y gana comisión sobre sí misma. Anti-abuso: `registerClient` compara `email_hash(client_email)` contra `email_hash` de la modelo referidora y sus alt-emails; si coincide → registra sin atribución + log de intento. Se decide en la subpasada 4 si se prioriza; opcional en fase inicial dado que el volumen de modelos es bajo y el operador puede detectar manualmente. **Prioridad**: media pre-go-live PROD.
