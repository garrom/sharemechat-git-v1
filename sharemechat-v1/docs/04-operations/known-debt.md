...
## 2026-09-02 — Teclado móvil tapa el chat (viewport + shell de altura fija)

**#D-61 — El teclado virtual móvil ocupa toda la pantalla en el chat y no restaura al cerrarse — ABIERTA (solo diagnóstico, sin implementar)**. Síntoma reportado por el operador: al enfocar el input del chat en móvil, el teclado se superpone tapando la vista y, al terminar de escribir, la pantalla queda desplazada/"ampliada" sin volver al estado previo. Afecta al chat de streaming (random/calling) y al chat puro (favoritos).

**Causa raíz (verificada en código fuente, no en docs):**
1. `frontend/public/index.html` (viewport meta) = `width=device-width, initial-scale=1, viewport-fit=cover` — **SIN la clave `interactive-widget`**. El valor por defecto `resizes-visual` hace que el teclado encoja solo el *visual viewport*, no el *layout viewport*: la página sigue midiendo lo mismo por debajo del teclado.
2. `frontend/src/styles/pages-styles/VideochatStyles.js` → `DashboardShell` (~L136-159): para los tabs app-like (videochat/favoritos/calling) el shell es `height: 100vh/100dvh` + `overflow: hidden`. Como el teclado no encoge el layout viewport, `100dvh` sigue valiendo la pantalla completa **detrás** del teclado y `overflow:hidden` impide reacomodar → el composer queda tapado y la vista visible arriba se ve comprimida.
3. La restauración fallida ("queda ampliada/desproporcionada") es el bug conocido de Safari iOS con layouts `100vh` fijos + teclado: el auto-scroll de foco (agravado por `scroll-margin-bottom: 30vh` en `StyledChatInput`, ~L906, mitigación "Fase A") desplaza el visual viewport y no se restaura al cerrar el teclado dentro de un shell `overflow:hidden`.

Mitigaciones ya presentes que se quedan cortas: `StyledChatInput` (~L882-909) tiene `font-size: 16px` (evita el auto-zoom de foco de iOS en ESE input — bien) y `scroll-margin-bottom: 30vh` (pelea contra el shell de altura fija → funciona a medias y deja la pantalla desplazada).

**Solución propuesta (por capas) — soporte real a sept 2026:**
- **Capa 1** (Android/Firefox, cero JS): añadir `interactive-widget=resizes-content` al viewport meta → `dvh`/`100dvh` **sí encogen** al abrir el teclado; el shell se reduce al espacio sobre el teclado y restaura al cerrar. Soporte: **Chrome 108+, Firefox 132+**. **iOS Safari NO lo soporta ni en iOS 26** (WebKit standards-position #65, abierta): en iOS el teclado solo encoge el visual viewport.
- **Capa 2** (el arreglo real de iOS): hook `useVisualViewport` que, con el input enfocado en el tab de chat, fija la altura del shell a `window.visualViewport.height` (en px, NO `100dvh`) y/o desplaza el composer por el offset, y **revierte en `blur`**. Debe ir **acotado al tab de chat** y evitar el `overflow:hidden` de `100dvh` fijo mientras hay foco.
- **Capa 3** (higiene): auditar `font-size ≥ 16px` en **todos** los inputs del flujo móvil (el del chat ya lo tiene; otros pueden no); composer como último hijo flex de la columna; único scroller = el de mensajes.
...
## 2026-08-14 — Deudas del catálogo de gifts (dato manual) y nivelación parcial de AUDIT

**#D-30 — Catálogo de gifts (`tabla gifts`) es dato MANUAL fuera de git → drift entre entornos; PROD sin catálogo — ABIERTA**. El seed del catálogo de regalos/emojis NO está en ninguna migración Flyway: `sharemechat-v1/src/main/resources/db/migration/V1__baseline.sql` crea la tabla `gifts` con `AUTO_INCREMENT=31` pero **sin los `INSERT`** (el baseline se llevó el esquema y descartó las filas). Consecuencia: cada entorno se siembra a mano y queda fuera de control de versiones → drift. Estado al 2026-08-14: **TEST** = 22 activos (fuente de verdad); **AUDIT** reconciliado a esos 22 el mismo día; **PROD con la tabla `gifts` VACÍA (0 filas)** → los regalos no funcionan en PROD. Nota importante: los iconos se pintan por `code` con SVG del frontend (`frontend/src/components/gifts/GiftIcon.jsx`), así que **NO hacen falta imágenes en S3** (el `gifts.icon` `.webp` es fallback no usado; los `.webp` de `assets.<env>/g/` son del render viejo). **Acción raíz**: crear una **migración Flyway `V__seed_gifts_catalog.sql`** con el catálogo canónico → paridad por construcción; el `icon` puede ser ruta relativa (resuelta con `app.assets.base-url`) o placeholder, ya que se pinta por code. **Acción inmediata PROD**: replicar el patrón usado en AUDIT (backend+frontend a `main` + seed de las 22 filas) cuando el operador lo valide (lo probará antes con su usuario modelo). FK a respetar al sembrar/reconciliar: `transactions.gift_id → gifts.id` (usar INSERT/UPDATE, NO DELETE). **Prioridad**: ALTA para PROD (bloquea los regalos en producción); MEDIA la migración de raíz. Ref: bitácora 2026-08-14 y memoria `project-gift-emoji-system`.

**#D-31 — Frontend admin de AUDIT sin nivelar (en la nivelación 2026-08-14 solo se desplegó product) — ABIERTA**. La nivelación de AUDIT a `main` del 2026-08-14 corrió `deploy-frontend.ps1 -Environment audit -Surface product`; el bundle **admin** de AUDIT sigue en el commit `bacb5cf` (2026-08-02). Para paridad total de AUDIT falta `-Surface admin` desde `main`. **Prioridad**: BAJA (no afecta a los regalos ni al producto público de AUDIT; solo al backoffice admin de AUDIT).

## 2026-08-04 — Deuda por evolutivo panel admin Master (ADR-056)

**#D-27 — Columna "Régimen (libre / bajo estudio)" en panel admin de modelos — CERRADA 2026-08-08**. Entregado tal como se propuso:
- Backend `AdminService.getModels` (`sharemechat-v1/src/main/java/com/sharemechat/service/AdminService.java`) carga en batch los `Model` records via `modelRepository.findAllById(userIds)` para obtener `masterUserId` de cada modelo, luego carga en batch los users master via `userRepository.findAllById(masterIds)` para obtener nickname (o email como fallback si nickname blank). Popula `dto.setMasterUserId` + `dto.setMasterNickname` cuando corresponde.
- DTO `UserDTO` extendido con `masterUserId` (Long) + `masterNickname` (String) — solo poblados en el flujo AdminService.getModels; en el resto de mapeos (`mapToDTO`) quedan null.
- Frontend `AdminModelsPanel.jsx` (bundle admin): (a) columna nueva "Régimen" entre "Cuenta" y "Acciones" con chip color-coded ("Libre" verde / "Bajo estudio: {master_nickname}" ámbar); (b) filtro tri-state en cabecera ("Todos" / "Libres" / "Bajo estudio") aplicado en cascada tras el filtro de estado. Client-side sobre la lista ya cargada (mismo pattern que los filtros existentes).
- i18n keys nuevas con fallback inline (`admin.models.filters.regimeLabel/regimeFree/regimeUnderStudio`, `admin.models.columns.regime`, `admin.models.regime.freeLabel/underStudioLabel/*Tooltip`).
- Cero cambios BD — `models.master_user_id` ya existía.
- Desplegado TEST 2026-08-08 con Surface=both (backend commit `35942bd`, admin bundle `main.86c259ba.js`, product bundle `main.0245d84c.js`).
- Deuda ADR-056 §D9 cerrada.

## 2026-08-08 — Deudas derivadas del brainstorming (A0004 / A0006)

**#D-28 — Madagascar (MG) NO está en allowlist de registro modelo — bloquea 2 modelos fundadoras vivas — ABIERTA**. Detectada durante el brainstorming del 2026-08-08 (`docs/_archive/brainstorming/artifacts/A0004.md` §v2). Estado actual: 2 hermanas malgaches contactadas y dispuestas a firmar como fundadoras junto con 1 modelo colombiana; las hermanas están bloqueadas hoy porque el env var `COUNTRY_ACCESS_MODEL_REGISTRATION_ALLOWED_COUNTRIES` en `/opt/sharemechat/config.env` de TEST/AUDIT/PROD no incluye `MG`. `CountryAccessService.assertAllowedForModelRegistration` lanza `CountryBlockedException` con mensaje uniforme "Registro no disponible" para IPs origen MG. **Viabilidad confirmada A0004**: Didit anuncia cobertura 220+ países (MG casi seguro incluido — confirmar con 1 KYC real antes de comprometer), payout viable por cripto (Paxum no cubre MG, tarjeta descartada), MG no aparece en listas de sancionados (UE/OFAC). **Acción operacional en TEST** (secuencia): (1) SSH TEST → editar `/opt/sharemechat/config.env` → añadir `MG` a `COUNTRY_ACCESS_MODEL_REGISTRATION_ALLOWED_COUNTRIES` → `sudo systemctl restart sharemechat-test` → verificar en logs de arranque que `CountryAccessService.modelAllowed` incluye MG; (2) test KYC Didit real end-to-end con documento malgache (CIN o pasaporte) + selfie desde una de las hermanas — confirma que Didit acepta el tipo de documento; (3) confirmar rail payout USDT TRC-20 desde el flow de retirada modelo (destino wallet MG); (4) replicar env var en AUDIT + PROD una vez validados 1-3. **Impacto residual**: cada semana de retraso son 2 fundadoras perdidas del pool inicial de 3 (Founding Models, ver §6.4 pending-hardening). **Prioridad**: ALTA — bloqueo directo de captación real. Ver plan derivado en `docs/07-roadmap/pending-hardening.md` §6.1.

**#D-29 — Documentación de reparto modelo desactualizada (75-79% obsoleto vs 50-60% real en BBDD PROD) — ABIERTA**. Detectada durante el brainstorming del 2026-08-08 (`docs/_archive/brainstorming/artifacts/A0004.md` §v2). `docs/01-business/sistema-tiers-modelos.md` y varios pasajes de ADR-052 declaran reparto modelo 75-79% (T1-T4 del rediseño estructural del 2026-07-24). **La realidad en BBDD PROD hoy es 50-60% (T1-T4)** tras el superseded por ADR-056 (2026-07-29) que reinstauró el motor de tramos con % INDIVIDUAL más conservador. Consecuencia: (a) el pitch "70% forever" para Founding Models (A0005 copy) solo tiene sentido leyendo el reparto REAL 50-60% como piso — con la doc antigua "70%" parece apenas +5pp sobre el estándar y pierde impacto; (b) discrepancia doc↔runtime confunde al operador al planificar tramos fundadoras; (c) contamina cálculos aguas abajo en `pricing.md`, `business-model.md`, `modelo-financiero.md`, `master-contract-v1-draft.md`. **Acción**: (1) reescribir `sistema-tiers-modelos.md` con reparto real 50-60% + tabla T1-T4 alineada con `TierService`/runtime + cabecera "Superseded ADR-052 por ADR-056 el 2026-07-29"; (2) auditar y actualizar referencias cruzadas al 75-79% en los 4 docs listados; (3) verificar que `AdminModelsPanel.jsx` y `ModelBillingPanel.jsx` renderizan los % reales de BD (no hardcoded pre-ADR-056). **Prioridad**: MEDIA — no bloquea operativa técnica, sí bloquea claridad al planificar cohorte fundadora. Ver plan derivado en `docs/07-roadmap/pending-hardening.md` §6.3.
...
**#D-24 — Rediseño de packs de recarga premium para modelos T3/T4 — PARCIALMENTE CERRADO 2026-07-25 (P100 añadido); RESIDUAL: cálculo dinámico minutos según chosenRate + agrupación UX picker**. Los packs vigentes hasta 2026-07-25 (10 / 20 / 40 €) estaban calibrados para tarifas cercanas a €1/min (T1). Con el rango de precio autoservicio introducido por [ADR-052 §D2](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md) (1-9 €/min según tramo), una modelo T4 con tarifa €9/min consumía el pack de 10 € en poco más de 1 minuto y el pack de 40 € en ~4,5 minutos. **2026-07-25**: añadido pack **P100 (100 EUR + 12 EUR bonus = 12%)** siguiendo el patrón escalado del catálogo (P10 0%, P20/P40 10%, P100 12%). Cubre 11-100 min según tarifa modelo (11 min a €9/min, 22 min a €5/min, 100 min a €1/min). Descartados P250/P500 por decisión del operador (no encajan con el posicionamiento de la plataforma). Cambios en 3 sitios canónicos del catálogo: `PspOrchestratorService.PACK_PRICES`, `PspWebhookOrchestratorService.PACK_BONUS`, `frontend/src/components/useAppModals.js` + `CheckoutSuccessPage.jsx`. **Residual pendiente**: (a) cálculo dinámico de la duración del pack en el picker según `chosenRateEurPerMin` de la modelo cuando el contexto lo tiene disponible (`random`/`calling`), en vez de mostrar minutos hardcoded a €1/min; (b) posible agrupación UX del picker (4 packs empiezan a ser ruidosos en el modal); (c) `recommended: true` contextual según tarifa modelo actual (hoy fijo en P20). **Impacto residual**: fricción de conversión sigue baja mientras no haya modelos T3/T4 activas ni tráfico real; el P100 desbloquea el caso de cliente que quiere sesión larga con modelo cara sin recargas continuas. **Prioridad**: media el residual UX/dinámica, baja mientras el volumen T3/T4 no lo justifique. **Referencia**: [`../01-business/pricing.md`](../01-business/pricing.md) sección "Fricción conocida entre rango de precio y packs vigentes".

**#D-25 — Recalibración del modelo financiero (`modelo-financiero.md` + xlsx) con nuevo reparto 75-79% — MD RECALIBRADO 2026-07-25; xlsx PENDIENTE**. La parte `.md` fue recalibrada a v2.0 el 2026-07-25: nuevos supuestos (75% modelo en T1, fees mix 50/50 cripto/tarjeta, coste trial €0.20/signup), nueva tabla de sensibilidad al mix, break-even revisado (~170 TX/mes en mix 50/50 vs ~35 TX/mes del modelo v1), hallazgos actualizados con foco en volumen como driver principal. El `.xlsx` sigue con la estructura v1 y lo recalibra el operador manualmente cuando disponga de tiempo, usando los supuestos que el `.md` v2.0 ya recoge. Mientras el xlsx no esté actualizado, el `.md` es la fuente de verdad estratégica. **Impacto residual**: el operador no tiene todavía la proyección mes-a-mes con celdas para pulsar supuestos y ver el escenario. **Prioridad**: baja (el `.md` cubre el marco estratégico; el xlsx es herramienta de simulación).

**#D-26 — T&C y contrato de modelo v5 con política de descuentos ADR-052 §D7 — BORRADOR CLAUSULAS FINALIZADO 2026-07-25 (7 cláusulas ES+EN + decisiones operador confirmadas + estado técnico documentado); PENDIENTE INTEGRAR EN v5 COMPLETO Y DESARROLLO SUB-FRENTE 3 TÉCNICO**. La política de responsabilidad económica introducida por [ADR-052 §D7](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md) — descuentos automáticos del payout siguiente por chargebacks/refunds/sanciones PSP atribuibles, umbral de suspensión 5% chargebacks/mes, derecho a disputa con evidencia, transparencia total en panel de la modelo, reserva del 5% primeros 90 días como colchón anti-chargeback — necesita **materializarse en T&C y contrato de modelo** (v5) con consentimiento explícito de la modelo al firmar. **Estado actual**: borrador textual de las 7 cláusulas nuevas (X.1 a X.7) finalizado en [`../01-business/model-contract-v5-clauses-d7-draft.md`](../01-business/model-contract-v5-clauses-d7-draft.md) en versión ES + EN por el asistente el 2026-07-25. Decisión explícita del operador: no se contrata asesoría legal externa; Shareme Technologies OÜ asume responsabilidad del texto final. Valores operador confirmados: umbral chargebacks 5% rolling 30d, notificación previa 7 días naturales, SLA disputa 10 días hábiles, arrastre 90 días naturales, reincidencia 2 en 12m → cancelación definitiva, reserva 5% primeros 90 días aplicable cripto+tarjeta indistintamente, jurisdicción heredada del v4. El borrador incluye sección "Estado de implementación técnica" que documenta: (a) infraestructura lista (Cláusula X.5 puntos 1-3 y Cláusula X.6 vía ModelContractManifestService); (b) no implementado (Cláusulas X.2, X.3, X.4, X.5 puntos 4-6, X.7 - todas dependen del Sub-frente 3 técnico); (c) 4 deudas técnicas confirmadas: (A) registro causa cierre sesión en stream_records; (B) estado UNDER_REVIEW en PayoutRequest; (C) tabla payout_deductions; (D) tabla payout_reserves. **Pendiente residual**: (a) refactor del Model Contract v4 completo integrando estas cláusulas (cubre deudas R5 + G4 del bloque 2026-06-27) — bump v5, publicación PDF, re-aceptación forzada 18 modelos vivas; (b) desarrollo del Sub-frente 3 técnico para pasar de ejecución manual a automática. **Publicable en v5 aunque la implementación técnica siga manual**: el valor de publicar el v5 ya es legitimar legalmente cualquier descuento manual que se aplique en el ínterin. **Prioridad Sub-frente 3 técnico**: dependerá del volumen real de chargebacks/refunds una vez PROD arranque con tráfico. Con solo cripto y volumen bajo, el manual es sostenible; con integración PSP tarjeta o primeros 5-10 chargebacks reales, urgente. Alineado con la deuda R5 "Model Collaboration Agreement v5 con lenguaje nuevo" ya declarada en el bloque de 2026-06-27 (esta deuda #D-26 refina el scope de R5 con los requisitos económicos concretos de ADR-052).

## 2026-07-24 — Deudas CANCELADAS por retirada del programa de afiliadas (ADR-052 §D11)

Bloque de deudas del ADR-049 canceladas por retiro del programa de afiliadas ([ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)). Ninguna se cierra por resolución; se cancelan porque el programa que las motivaba ya no existe.

- **#D-18 (Onboarding B2B de entidades externas) — CANCELADA**. Sin programa de afiliación estándar, no hay onboarding B2B a extender. Los acuerdos B2B se negocian caso por caso fuera del régimen estándar.
- **#D-19 (Cooldown antifraude automático por chargeback) — CANCELADA**. Sin programa, no hay `affiliate_id` que suspender.
- **#D-20 (Webhook PSP para chargebacks/refunds automáticos) — CANCELADA en su vertiente de afiliadas**. El webhook PSP sigue siendo necesario para el ciclo económico general (refunds de packs, chargebacks aplicados a payouts de modelo según [ADR-052 §D7](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)), pero ya no como reverso de comisiones de afiliación.
- **#D-21 (Panel admin de afiliaciones) — CANCELADA**. Sin programa, no hay panel que construir.
- **#D-22 (Detección de auto-referral) — CANCELADA**. Sin atribución cliente↔modelo referidora, no hay vector de auto-referral que detectar.
- **#D-23 (Copy REFERRAL_MAGIC_LINK y REFERRAL_INVITATION) — CANCELADA**. Los emails de invitación al cliente referido no se enviarán; las plantillas del `EmailCopyRenderer` correspondientes se retiran junto con el resto del código en el Sub-frente 2 técnico.

## 2026-07-11 — Deuda abierta en la Subpasada 2B del ADR-049

**#D-23 — Copy REFERRAL_MAGIC_LINK y REFERRAL_INVITATION pendiente de revisión editorial — CANCELADA 2026-07-24 por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. La Subpasada 2B del programa de afiliadas introdujo dos claves nuevas del `EmailCopyRenderer` (`renderReferralMagicLink` y `renderReferralInvitation`) con placeholders técnicos ES + EN suficientes para arrancar el pipeline y pasar el smoke en TEST. Los textos NO son editorialmente definitivos; su vocabulario, tono y llamadas a la acción deben ajustarse antes de campaña real de captación. El operador ha aprobado dejarlos como están para no bloquear la subpasada, con la deuda registrada aquí. **Acción pendiente**: revisar copy junto con el material marketing del programa de afiliadas (fase pre-lanzamiento a modelos). Los cambios son puramente textuales; no requieren cambio de firma ni schema. **Prioridad**: media antes del outreach real a modelos (business `affiliate-program.md`). **Cancelación**: el programa de afiliadas queda retirado por completo en [ADR-052](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md); las plantillas de email correspondientes se retiran junto con el resto del código en el Sub-frente 2 técnico del Frente 3.

## 2026-07-11 — Deudas diferidas al cierre del ADR-049 (Programa de afiliadas)

**#D-18 — Onboarding B2B de entidades externas (blogs, agencias, estudios) — CANCELADA 2026-07-24 por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` declara que solo modelos con KYC APPROVED pueden activar el programa de afiliadas en la fase inicial. Las entidades externas descritas en `../01-business/affiliate-program.md` §3-§6 quedan fuera del alcance. Cuando aparezca demanda concreta se abre frente propio con revisión manual, terms específicos y extensión de `affiliate_codes` (nullable `model_user_id` + `external_affiliate_id`) o tabla independiente. **Prioridad**: baja hasta que el operador tenga interlocutor B2B concreto. **Cancelación**: sin programa estándar, los acuerdos B2B se negocian caso por caso fuera del régimen estándar (contabilidad manual, no plataforma).

**#D-19 — Cooldown antifraude automático por chargeback — CANCELADA 2026-07-24 por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. `../01-business/affiliate-program.md` §4 fija cooldown >3% chargeback sostenido 3 meses como criterio de suspensión. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D9 sostiene que en la fase inicial el operador suspende manualmente vía endpoint admin (`POST /api/admin/affiliate/deactivate`). La automatización con detección + suspensión + revisión manual queda como sub-fase posterior cuando el volumen justifique. **Prioridad**: media pre-go-live PROD. **Cancelación**: sin programa, no hay `affiliate_id` que suspender. La política de chargebacks aplicada a payouts de modelo por sesiones específicas (ADR-052 §D7) es un frente distinto y se materializa aparte.

**#D-20 — Webhook PSP para chargebacks/refunds automáticos — CANCELADA 2026-07-24 en su vertiente de afiliadas por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D8 materializa el reverso de comisión vía endpoint admin manual (`POST /api/admin/affiliate/chargeback`). En la fase actual (soft launch cripto vía Paxum/NOWPayments, ADR-047) no hay ventana de chargeback tarjeta y el flujo manual basta. Cuando el PSP tarjeta esté integrado con webhook de chargebacks, el motor de comisiones lo consume automáticamente. **Prioridad**: alta cuando se integre PSP tarjeta. **Cancelación (parcial)**: el reverso de comisiones de afiliadas se cancela junto con el programa; el webhook PSP sigue siendo necesario para el ciclo económico general (refunds de packs, chargebacks aplicados a payouts de modelo según [ADR-052 §D7](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)). Esa deuda se replantea aparte cuando toque integrar PSP tarjeta.

**#D-21 — Panel admin de afiliaciones — CANCELADA 2026-07-24 por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` D13 subpasada 8 lo declara opcional en la fase inicial. Sin él, el operador consulta por SQL directo el estado de las comisiones y los payouts filtrando por `payout_requests.payout_type='AFFILIATE'`. Panel React con listado, comisiones agregadas, chargebacks, alertas de cooldown se abre como frente propio cuando el volumen lo justifique. **Prioridad**: baja hasta que haya ≥5 afiliadas activas. **Cancelación**: sin programa, no hay panel que construir. El panel admin de descuentos a modelos por chargebacks/refunds (ADR-052 §D9) es un frente distinto y se materializa aparte en el Frente 3 Sub-frente 3.

**#D-22 — Detección de auto-referral — CANCELADA 2026-07-24 por [ADR-052 §D11](../06-decisions/adr-052-rediseno-reparto-precio-y-retirada-afiliadas.md)**. `../06-decisions/adr-049-programa-afiliadas-modelos.md` deudas diferidas: una modelo comparte su URL, se auto-registra como cliente con otra cuenta y gana comisión sobre sí misma. Anti-abuso: `registerClient` compara `email_hash(client_email)` contra `email_hash` de la modelo referidora y sus alt-emails; si coincide → registra sin atribución + log de intento. Se decide en la subpasada 4 si se prioriza; opcional en fase inicial dado que el volumen de modelos es bajo y el operador puede detectar manualmente. **Prioridad**: media pre-go-live PROD. **Cancelación**: sin atribución cliente↔modelo referidora, no hay vector de auto-referral que detectar.

## 2026-07-09 — Deudas detectadas durante auditoría exhaustiva de código + nivelación AUDIT

**#D-16 — Ventana no-atómica entre `TransactionService.processGift` y `MessageService.sendGift` en el envío de gift por WebSocket**. Detectada durante la verificación V6 de la auditoría exhaustiva de código previa a la nivelación AUDIT (2026-07-09). En `MessagesWsHandlerSupport.handleMsgGift` (líneas `786-790`), la rama CLIENT→MODEL ejecuta primero `transactionService.processGift(me, to, giftId, resolvedCallingStreamId)` (o `processGiftInChat` si no hay contexto de sesión) — método `@Transactional` que commitea charge del cliente + earning del modelo + `STREAM_MARGIN` de plataforma —, y **después**, ya fuera de esa transacción, llama a `messageService.sendGift(me, to, g)` — método `@Transactional` independiente que inserta la fila en `messages` con `gift_id`. Si `sendGift` lanza excepción entre las dos transacciones (fallo transitorio de BD, lock timeout, deadlock), el ledger queda descontado sin fila persistida en `messages`. El cliente recibe `msg:error` y no ve confirmación visible del gift, pero el saldo se ha ido. Probabilidad esperada bajo carga normal es baja (`sendGift` es un `INSERT` simple), pero rompe la promesa "saldo descontado ⇔ gift entregado". **Resolución sugerida**: envolver ambas operaciones en un único método coordinador `@Transactional` — cambio pequeño y localizado en `MessagesWsHandlerSupport` o en un servicio nuevo tipo `GiftMessagingService`. **Prioridad**: baja hoy; subir a media cuando el volumen de gifts en operación real justifique el hardening. **Referencia factual**: `TransactionService.java:621-622` (`processGift @Transactional`), `TransactionService.java:1045-1047` (`processGiftInChat @Transactional`), `MessageService.java:68-69` (`sendGift @Transactional`), `MessagesWsHandlerSupport.java:786-790` (la secuencia sin coordinador).

**#D-17 — Documentación del `.env` del EC2 (config.env + secrets.env) como fuente autoritaria de configuración runtime** — **CERRADA en el mismo commit por sección nueva en `docs/04-operations/access-and-tooling.md`**. Detectada porque en sesiones recientes, cuando el agente lee la documentación para ponerse al día, descubre que el `.env` del EC2 NO estaba explicado como fuente autoritaria de configuración por entorno. La documentación implicaba que todo vivía en `application-*.properties` del repo, cuando en realidad los flags críticos por entorno (registro OFF, Didit OFF, Sightengine OFF, `CLAUDE_API_KEY`, `AUTHRISK_ENABLED`, `KYC_DIDIT_ENABLED`, `MODERATION_SIGHTENGINE_ENABLED`, `COUNTRY_ACCESS_ENABLED`, `PRODUCT_REGISTRATION_*_ENABLED`, `PRODUCT_ACCESS_MODE`, `PRODUCT_SIMULATION_TRANSACTIONS_DIRECT_ENABLED`, etc.) viven en el `.env` del EC2 y sobrescriben los defaults del repo. La confusión llevó en la auditoría del 2026-07-09 a etiquetar como "BUGS DE SEGURIDAD CRÍTICOS" features apagadas intencionalmente, solo porque los `application-*.properties` del repo no las override y los defaults son permisivos. **Resolución aplicada**: sección nueva en `access-and-tooling.md` explicando (a) que `/opt/sharemechat/config.env` (no sensible, `root:root 0644`) y `/opt/sharemechat/secrets.env` (sensible, `0600 root:root` o `ec2-user:ec2-user` según entorno) son cargados por systemd como `EnvironmentFile`; (b) que Spring lee variables de entorno **antes** de aplicar defaults de `application-*.properties`; (c) qué tipo de flags viven en cada sitio (defaults compartidos por entorno + comportamiento del código en el repo, secretos + flags operativos por entorno en el `.env` del EC2); (d) ejemplos concretos por categoría; (e) la regla operativa: antes de afirmar "feature X está apagada / activada", contrastar con el `.env` del EC2 correspondiente vía SSH, no solo con `application-*.properties` del repo. Prioridad: documental (aplicada).

## 2026-07-08 — Deudas diferidas al cierre de B.3.1 (ADR-046)

**#D-13 — Job diario de expiración `ESCALATED > 48h`**. Sin agentes conectados, un caso escalado a las 03:00 se queda indefinidamente en `ESCALATED` y el user espera respuesta que nunca llega. Falta un job que mueva `ESCALATED > 48h` (parametrizable) a `ABANDONED` (o `EXPIRED` nuevo) e inserte un mensaje SYSTEM al user tipo "tu caso quedó sin atender, por favor vuelve a escribir". Prioridad media, planificable en B.3.4 o antes si aparece el caso en operación real. Tabla involucrada: `support_conversations` con filtro `resolution_status='ESCALATED' AND assigned_agent_id IS NULL AND escalated_at < NOW() - INTERVAL 48 HOUR`.

**#D-14 — Browser Notification API para agentes conectados al panel admin**. El operador descartó email como canal de aviso al equipo humano. El único señal hoy es el badge del panel, que exige tenerlo abierto. Añadir `Notification.permission` opt-in dispara notificaciones nativas del navegador cuando el badge sube. Sin infra push server, sin coste. Prioridad media. **Estado 2026-07-08**: originalmente prevista para B.3.2 según ADR-046 al abrirse la deuda, finalmente **diferida** — no incluida en el alcance final entregado de B.3.2 (commit `794193d`). Sigue viva; ejecutar en B.3.4 o en sub-sesión propia. Verificado por grep `Notification.permission|Notification.request|Notification(` en `frontend/src/` sin resultados el 2026-07-08 tras el deploy admin.

**#D-15 — Playbook DPO para GDPR art. 15 sobre conversaciones humanas**. Cuando un user ejercita derecho de acceso y pregunta "¿quién me atendió?", la política canónica de ADR-046 es devolver únicamente el `display_name` de la profile (máscara pública), **jamás** el `user_id` real del agente. Con la Decisión B (profiles compartidas por múltiples users reales), esto pasa de recomendable a obligatorio — no se puede ligar "Pepito" a Juan Pérez ante el user porque puede haber sido Juan o Marta. Falta el documento formal en el corpus legal explicando el flujo de respuesta a la solicitud, la retención mínima del `sent_by_user_id` para auditoría interna, y los canales bajo los que sí se revela (orden judicial, requerimiento del PSP). Prioridad media pre-go-live PROD; obligatoria antes de abrir el panel a producción.

## 2026-07-07 — CERRADA: secrets.env de TEST con ownership `root:root` desde 2026-07-02

**#D-12 — Ownership divergente en `/opt/sharemechat/secrets.env` de TEST — CERRADA 2026-07-07**. Detectada durante el smoke Fase 1.C del refactor Agente IA (ADR-044). El fichero pasó a `-rw------- root:root` con change time `2026-07-02 15:28:43 UTC` (probablemente durante alguna operación de la Fase B.1 del sub-paquete Chat Soporte LLM). El state-mapping local documenta la política canónica de TEST como `0600 ec2-user:ec2-user` (comentario textual: "asimetría vs AUDIT/PROD por arranque manual"). Consecuencia inmediata: `start-test.sh` corre como `ec2-user`, hace `source /opt/sharemechat/secrets.env` y no puede leerlo → el backend no arranca con el patrón canónico y solo sobrevive el proceso previo (PID que aún tenía las env vars cargadas de una lectura anterior). Detectado hoy al matar el proceso vivo (PID 1509) y no poder relanzar: el log capturó `./start-test.sh: line 29: /opt/sharemechat/secrets.env: Permission denied`. **Fix aplicado**: `sudo chown ec2-user:ec2-user /opt/sharemechat/secrets.env` restaura el ownership al documentado, conservando `0600`. Backend arrancó limpio a continuación (Started en 32s, PID 5596). **Ventana de exposición**: 5 días (2026-07-02 → 2026-07-07) en los que un reinicio del backend TEST habría fallado silenciosamente si no había sesión SSH activa; el proceso vivo llevaba env vars cargadas desde antes del cambio y por eso no había síntoma visible. **Aprendizaje**: cuando se toque `/opt/sharemechat/secrets.env` de TEST vía sudo, restaurar ownership `ec2-user:ec2-user` al final. Nada que replicar en AUDIT/PROD (ambos operan bajo systemd como `ec2-user` con `EnvironmentFile=`; el owner `root:root` de los secrets no bloquea porque systemd los lee como root antes de dropear privilegios). **Referencia factual**: descubierto durante smoke Fase 1.C ADR-044, entrada de bitácora 2026-07-07 correspondiente.

## 2026-07-06 — PROD hardening coming-soon (nivelación ADR-045 → PROD)

Bloque de tres deudas relacionadas con el estado deliberadamente apagado de Didit y Sightengine en el entorno PROD durante la fase coming-soon del producto. Aplicado en el commit del hardening pre-nivelación PROD (2026-07-06) como capa de defensa-en-profundidad + documentación explícita. TEST y AUDIT NO cambian: sus `application-*.properties` no llevan override y sus `/opt/sharemechat/secrets.env` mantienen credenciales reales.

**#D-9 — Didit KYC apagado en PROD durante coming-soon**. `application-prod.properties` fija `kyc.didit.enabled=false` con env vars `KYC_DIDIT_API_KEY / API_SECRET / MODEL_WORKFLOW_ID / CLIENT_WORKFLOW_ID` referenciadas pero pobladas como placeholder `disabled` en `/opt/sharemechat/secrets.env` del EC2 PROD. Efecto: los registros de modelo (`POST /api/users/register/model`) persisten al usuario en BD con `verification_status=PENDING` sin disparar Didit real (el flujo de registro no llama a Didit, confirmado por trazado del fuente 2026-07-06). El flujo posterior de KYC (`/api/models/kyc/*`) sí invoca `DiditClientImpl.createSession()` pero el guard interno de líneas 47-61 devuelve un mock estable apuntando a `https://verification.test.sharemechat.com/mock/didit/...` — URL de TEST no válida en PROD, experiencia rota si el modelo pulsa el botón "Iniciar verificación". Impacto para el usuario final: el modelo puede registrarse y esperar activación futura del KYC, pero si intenta iniciar la verificación se le redirige a un dominio TEST. **Al activar Didit en PROD**: (1) reemplazar los cuatro placeholders `disabled` en `/opt/sharemechat/secrets.env` de PROD por los valores reales del panel Didit; (2) poner `KYC_DIDIT_ENABLED=true` en `/opt/sharemechat/config.env` de PROD (o retirar el override falso de `application-prod.properties`); (3) verificar que la fila `kyc_provider_config` para modelo tiene `active_mode=DIDIT`; (4) smoke del botón "Iniciar verificación" del dashboard modelo con manejo UX si Didit responde error transitorio. Alternativa mientras dure el coming-soon: ocultar el botón "Iniciar verificación" del dashboard modelo vía feature flag frontend para evitar la redirección rota a TEST. **Prioridad**: media, se resuelve al lanzamiento PROD post-coming-soon.

**#D-10 — Sightengine (moderación IA visual) apagado en PROD durante coming-soon**. `application-prod.properties` fija `moderation.sightengine.enabled=false` con env vars `MODERATION_SIGHTENGINE_API_USER / API_SECRET / WORKFLOW_ID / WEBHOOK_SECRET` referenciadas pero pobladas como placeholder `disabled` en `/opt/sharemechat/secrets.env` del EC2 PROD. Efecto: la moderación de streams sigue usando `MockModerationClient` (seed inicial de V10 con `active_mode=MOCK` en `stream_moderation_provider_config`); `SightengineModerationClient` se instancia como bean sin `@ConditionalOnProperty` pero no se selecciona en runtime. Endpoint webhook público `POST /api/webhooks/moderation/{vendor}` queda expuesto (no condicionado) pero responde `401 webhook_secret_not_configured` si un scanner externo lo llama — comportamiento controlado y documentado como stub P1.3 en el propio controller. **Al activar Sightengine en PROD**: (1) reemplazar los cuatro placeholders `disabled` en `/opt/sharemechat/secrets.env` de PROD por credenciales reales; (2) poner `MODERATION_SIGHTENGINE_ENABLED=true` en `/opt/sharemechat/config.env` de PROD; (3) cambiar `active_mode` a `SIGHTENGINE` en `stream_moderation_provider_config` vía endpoint admin; (4) smoke de un stream real con verdicts GREEN/YELLOW/RED según severidad. **Deuda relacionada #D-11**: el endpoint webhook público está expuesto siempre en cualquier entorno; no bloquea el hardening pero conviene evolucionarlo a `@ConditionalOnProperty` cuando el proyecto tenga más superficie de vendors (P2 postponed). **Prioridad**: media, al lanzar moderación real.

**#D-11 — Credenciales de PROD como placeholders "disabled" en secrets.env, no reales**. `/opt/sharemechat/secrets.env` de PROD contiene ocho variables `KYC_DIDIT_*` y `MODERATION_SIGHTENGINE_*` pobladas con el valor literal `disabled` (2026-07-06). Están ahí para que Spring resuelva el `${VAR:}` binding sin lanzar excepción al arrancar, NO para que el runtime las consuma (los servicios están apagados por `enabled=false` en `application-prod.properties`). Efecto: si por error alguien activara `enabled=true` sin sustituir el placeholder, Spring pasa binding pero el guard interno del cliente Didit/Sightengine rechaza inmediatamente (`isBlank` no aplica a "disabled" que NO es blank; el cliente Sightengine sí lanzaría `IllegalStateException`, el cliente Didit sí ejecutaría el POST real con `x-api-key: disabled` que Didit respondería 401). **Al activar Didit y/o Sightengine reales en PROD**: reemplazar los placeholders por credenciales reales ANTES de flipar `enabled=true`. Documentar la operación en `incident-notes.md` cuando se ejecute. **Prioridad**: media, ligada al ciclo #D-9 / #D-10. **Referencia factual**: bloque HARDENING PROD coming-soon en `/opt/sharemechat/secrets.env` de PROD (permisos 0600 root:root, tamaño 1449 bytes tras el append de 2026-07-06 09:56 UTC).

## 2026-07-05 — Contrato roto en el bootstrap de translation EN (bloquea ADR-045 subpasada 2C)

**#D-8 — Contrato roto entre javadoc de `updateTranslationBody` y schema V2: no hay forma de poblar translation EN antes del primer apply-bilingual** — **CERRADA por [ADR-045 subpasada 2C.0]** (commit del feat que introduce `POST /admin/content/articles/{articleId}/translations` + fix del `.orElseGet` de `updateTranslationBody`; la creación on-demand desaparece y la translation se instancia vía el endpoint nuevo con `slug` y `title` obligatorios). Descubierto durante el smoke E2E post-ADR-045 subpasada 2B (2026-07-05, articleId 9 en TEST). El javadoc de `ContentArticleService.updateTranslationBody` (líneas 296-341) promete creación on-demand de la translation EN cuando el operador hace `PUT /articles/{id}/translations/en/body` antes de haber pasado por `apply-bilingual`, con la nota literal "la translation nace con slug y title placeholder vacios (a llenar por apply-bilingual o por endpoint dedicado en paquete 6)". El schema V2 lo impide: `content_article_translations.slug VARCHAR(160) NOT NULL` y `title VARCHAR(255) NOT NULL`. Al intentar la creación real el backend responde HTTP 500 con `java.sql.SQLIntegrityConstraintViolationException: Column 'slug' cannot be null` (confirmado en `/tmp/sharemechat-test.log` del EC2 TEST). El "endpoint dedicado en paquete 6" citado en el javadoc **no existe**. `PATCH /articles/{id}/translations/en` con las keywords directamente **también falla** porque la fila no existe (`updateTranslationMetadata` línea 386 lanza 404, que llega al cliente enmascarado como SPA fallback por el CloudFront del subdominio admin — mismo patrón conocido que la discrepancia #D-6 del bloque anterior). **Impacto**: BLOQUEA la subpasada 2C del ADR-045. La UI de 2C debe permitir al operador poblar primary/secondary keywords EN antes del pipeline (ADR-045 D3: "Primary EN opcional... Si el operador la rellena, la fase 4.5 la honra sin sustituir"). Sin endpoint para instanciar la translation EN, esa promesa del ADR no aterriza en el flujo real; el operador sólo puede declarar keywords EN post-`apply-bilingual`, momento en que la IA ya decidió. **Referencia factual**: descubierto 2026-07-05 durante smoke E2E post-2B; articleId 9 en TEST (queda vivo para paso C del smoke); logs `/tmp/sharemechat-test.log` en `test-backend`. **Resolución sugerida**: nueva subpasada 2B.5 (o previa a 2C) que añada endpoint `POST /admin/content/articles/{id}/translations` que crea una translation en un locale con `slug` + `title` obligatorios, opcionalmente `brief`, `seoTitle`, `metaDescription`, `primaryKeyword` y `secondaryKeywords`. Semántica: 409 CONFLICT si ya existe la translation (upsert añade complejidad al contrato de eventos EDIT_APPLIED). Alternativa descartada: relajar constraints `slug NOT NULL` / `title NOT NULL` con Flyway destructiva; rompe invariantes que otras zonas del código asumen (validación de transición DRAFT→IN_REVIEW ya exige ambos no vacíos; el modelo satélite ADR-025 asume que una translation existente es "hija completa" del artículo lógico). **Prioridad**: alta, bloqueante para 2C.

## 2026-07-04 — Discrepancias documentación vs código detectadas en el subsistema CMS (informe base de ADR-045)

Bloque de siete discrepancias entre ADRs vigentes y estado real del código del subsistema CMS, detectadas durante el análisis previo a [ADR-045](../06-decisions/adr-045-keywords-seo-per-locale.md). Base factual: `docs/_archive/analysis/2026-07-04-cms-articulo-keywords-por-locale.md` sección 6. Ninguna es bug funcional; todas son deuda documental que induce a error a cualquier lector que consulte los ADRs sin conocer las capas posteriores. Se agrupan aquí para no perderlas y para poder resolverlas en tandas cuando el operador priorice el saneamiento documental del subsistema CMS.

**#D-1 — Modelo satélite bilingüe superseded silente en ADRs 022 y 024**. [ADR-022](../06-decisions/adr-022-blog-cms-multilingual-es-en.md) describe el modelo bilingüe con `group_id UUID` vinculando ES/EN; [ADR-024](../06-decisions/adr-024-bilingual-submit-inside-editor.md) introduce `parent_article_id` como FK auto-referente. Ambos fueron sustituidos por el modelo satélite (`content_article_translations` como fila per-locale del mismo `content_articles.id`) en [ADR-025](../06-decisions/adr-025-flyway-introduction-and-cms-v2-schema.md), pero ninguno de los dos ADRs anteriores lleva marca visible de "superseded by ADR-025". Un lector nuevo saca conclusiones erróneas sobre el modelo de datos actual. **Impacto**: documental medio; puede confundir en revisiones futuras. **Resolución sugerida**: añadir cabecera "Superseded by [ADR-025]" en el bloque de estado de ambos ADRs. Priorización: baja.

**#D-2 — Opt-out "skip translate-en" documentado en ADR-023 pero no vivo en la skill** — **CERRADA por [ADR-045](../06-decisions/adr-045-keywords-seo-per-locale.md)**. [ADR-023](../06-decisions/adr-023-bilingual-editorial-pipeline-es-en.md) sección D3 describe una salida operativa: si el operador incluye "skip translate-en" al lanzar el pipeline, la fase 4.5 se salta y solo se emite JSON ES. La skill actual `cms-orchestrator.md` declara la fase 4.5 como obligatoria sin opt-out. Contradicción de contrato entre ADR y skill viva. **Resolución**: [ADR-045](../06-decisions/adr-045-keywords-seo-per-locale.md) sección "Impacto sobre ADRs anteriores" formaliza que el opt-out queda retirado (superseded parcial). Ningún trabajo adicional pendiente en este eje concreto.

**#D-3 — `SCHEDULED` modelado en BD pero muerto en el código**. `content_articles.state` acepta el valor `SCHEDULED` en el CHECK de la migración `V2__cms_v2_schema.sql` ([ADR-025](../06-decisions/adr-025-flyway-introduction-and-cms-v2-schema.md)). El servicio `ContentArticleService.transition` **no** contempla `SCHEDULED` en sus transiciones permitidas ni existe endpoint que active este estado. [ADR-016](../06-decisions/adr-016-content-workflow-simplification-and-retraction.md) sección D5 admite explícitamente que `SCHEDULED` se modela pero se difiere, así que la deuda es intencional; falta anotarla en algún sitio localizable por grep. **Impacto**: operativo bajo (nadie puede alcanzar el estado), documental medio. **Resolución sugerida**: añadir comentario SQL a la columna `state` señalando que `SCHEDULED` es placeholder pre-implementación, o eliminarlo del CHECK hasta que se necesite. Priorización: baja.

**#D-4 — `sources_used` como columna implicada en ADR-025 pero no persistida en BD**. [ADR-025](../06-decisions/adr-025-flyway-introduction-and-cms-v2-schema.md) sección D3 al listar los campos de `content_articles` cita "sources_used" entre las columnas del rediseño CMS v2. La migración real `V2__cms_v2_schema.sql` **no** crea esa columna; `sources_used` sólo vive en el bloque `shared` del JSON del pipeline en tiempo de ejecución, sin persistencia dedicada en tabla propia. La skill `cms-json-builder` sí lo emite en JSON, y `ContentRunService.applyBilingual` lo consume, pero no queda historizado en BD. **Impacto**: mayoritariamente documental (letra del ADR promete algo que no aterrizó); operativo bajo (los datos existen en el JSON aplicado, no en columna). **Resolución sugerida**: aclarar en ADR-025 que `sources_used` viaja por JSON y no por columna, o abrir columna dedicada si se decide auditarlo históricamente. Priorización: baja.

**#D-5 — Simplificación de "dos JSONs" a "un único JSON" no ADR-eada**. [ADR-022](../06-decisions/adr-022-blog-cms-multilingual-es-en.md) sección D9 y [ADR-023](../06-decisions/adr-023-bilingual-editorial-pipeline-es-en.md) sección D2 hablan de emitir dos ficheros separados `final_es.json` + `final_en.json`. El pipeline vivo hoy (skills `cms-json-builder` y `cms-orchestrator`) emite **un único** `final.json` con estructura `{shared, locales.{es,en}}` schema 2.0. La simplificación probablemente aterrizó en el paquete de rediseño CMS v2 pero no hay ADR que la declare explícitamente. **Impacto**: documental medio; los ADRs 022/023 quedan factualmente desactualizados. **Resolución sugerida**: nota en ADR-025 (o ADR-026) confirmando la unificación a un único JSON. Priorización: baja.

**#D-6 — Renombrado del endpoint bilingüe sin ADR de corrección**. [ADR-024](../06-decisions/adr-024-bilingual-submit-inside-editor.md) documenta `POST /admin/content/articles/{id}/runs/{runId}/output-bilingual` con DTO `SubmitOutputBilingualRequest(rawJsonEs, rawJsonEn, ...)` (dos JSONs separados, coherente con D-5). El endpoint real es `POST /admin/content/articles/{articleId}/runs/{runId}/apply-bilingual` con body `{ rawJson, modelId, modelVersion? }` (un solo JSON). El nombre, el shape del DTO y la firma cambiaron sin ADR. **Impacto**: documental alto para nuevos contribuidores que busquen el endpoint por nombre; operativo cero (el endpoint funciona). **Resolución sugerida**: nota en ADR-024 con el nombre real vigente, o mini-ADR que documente el renombrado. Priorización: media (impacta descubrimiento del código).

**#D-7 — Workflow de 6 estados en ADR-010 nunca aterrizó**. [ADR-010](../06-decisions/adr-010-internal-content-cms-ai-assisted-workflow.md) describe el workflow editorial con 6 estados: IDEA → OUTLINE_READY → DRAFT_GENERATED → IN_REVIEW → APPROVED → PUBLISHED. Implementado hoy con 4 estados (`DRAFT → IN_REVIEW → PUBLISHED → RETRACTED`) definidos en [ADR-016](../06-decisions/adr-016-content-workflow-simplification-and-retraction.md). ADR-010 no lleva marca de superseded en la sección afectada. **Impacto**: documental medio; el resto del ADR-010 (asistencia IA, tablas satélites, workflow como concepto) sigue siendo la referencia canónica. **Resolución sugerida**: nota en la cabecera de ADR-010 delimitando qué partes siguen vivas y remitiendo a ADR-016 para el workflow. Priorización: baja.
...
**Cerrada en**: 2026-05-20 (paquete 10.A.0), antes de iniciar la nivelación de AUDIT. Cambio aplicado en CloudFront TEST `E2Q4VNDDWD5QBU` con `aws cloudfront update-distribution` (ETag pre `E2NEU26H0UBU3V`, ETag post `E1Z8RZ5B6MIFUG`). El behavior `/.well-known/acme-challenge/*` pasa de `Managed-CachingOptimized` (`658327ea-…`) a `Managed-CachingDisabled` (`4135ea2d-…`). Distribución alcanzó `Deployed` en ~3 min. Validación post-cambio confirma `Managed-CachingDisabled` aplicado y la ruta sigue sirviendo desde el origen (`api-test-backend`). Detalle en [incident-notes.md](docs/_archive/04-operations/incident-notes.md) sección "Cierre deuda cache policy /.well-known/acme-challenge/* en CloudFront TEST". Pendiente verificar el mismo behavior en AUDIT y PRO al inventariarlos, replicando el cambio si aplica.
...
### [CERRADA] Carpetas docs/_archive/state-inventory-skills/ y docs/_archive/_snapshots/ no registradas en governance
