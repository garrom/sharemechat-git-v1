...
### [DEUDA — moderación streaming] 7 deudas estratégicas tras cierre de Sightengine live en TEST

Tras el cierre de los sub-paquetes P2.1 (commit `9d2662c`, vendor real activo en TEST) y P2.2 (commit `3e97839`, delegación de decisión AMBER/RED al `summary.action` del workflow Sightengine), quedan abiertas siete deudas estratégicas que NO viven en bitácora porque son **transversales** al frente y bloquean trabajos no técnicos del proyecto (compliance Segpay, comunicaciones, refactor de policies, activación entornal). Las deudas tácticas (latencia métricas, pool S3 separado, panel preview imagen, etc.) sí viven en la bitácora del 2026-06-27.
...
1. **Refactor de policies públicas para alinear con la operación real**. El operador confirmó en P2.2 el posicionamiento operativo "adult dating 1-a-1 con nudity consensual entre adultos verificados permitido", alineado con AN 5196 + Visa Rule ID 0003356 + práctica CooMeet/LuckyCrush. Los documentos draft de policies en `docs/01-business/policies/*` (Content Management Policy v1.1, Safety and Community Guidelines, Terms and Conditions, Complaint & Content Removal Policy, Consumer Age Verification Policy, Chargeback & Fraud Mitigation Policy) están redactados con lenguaje más restrictivo del que refleja la operación real. **No es un cambio de software**: es trabajo de redacción legal/marca. Requiere coordinación con asesoría legal antes de publicar como ToS efectivos. **Prioridad alta** pre-go-live PROD (es entregable de onboarding Segpay según `compliance-deliverables.md` §2). Mientras tanto, el adapter en TEST opera correctamente delegando al workflow del dashboard, donde el operador ya configuró la política granular real.

2. **Comunicación clarificadora a Patricia Ucros (Segpay)**. El operador identificó en P2.2 que el posicionamiento que Segpay maneja sobre SharemeChat puede no estar alineado con la realidad operativa "adult dating tolerada" vs "adult cam más restrictivo". Acción pendiente: redactar y enviar comunicación dejando explícito el posicionamiento real, alineado con AN 5196 y con la práctica de comparables del vertical. **Prioridad alta** porque cualquier malentendido en la fase de due diligence con el PSP puede traducirse en exigencias técnicas que ya no aplican (refactorar mapper a hardcoded de nuevo, por ejemplo). Riesgo si NO se hace: contradicción entre lo que el operador comunica verbalmente al PSP y lo que las policies públicas del producto declaran tras refactor.

3. **Workflow formal de complaints con SLA 5 días hábiles**. Cualquier persona del público (no solo usuarios registrados) debe poder presentar quejas sobre contenido o sobre actuaciones de la plataforma, con plazo de respuesta comprometido (5 días hábiles según el régimen Segpay; ver `compliance-deliverables.md` §2.3). Hoy existe parcialmente `/api/reports/abuse` para usuarios autenticados, sin superficie pública ni cola admin formal ni audit log de tiempos. Bloqueante para cerrar onboarding Segpay y para alinear con UE DSA art. 28. Estimación: sub-paquete dedicado de 2-4 sesiones.

4. **Reporting mensual + nil report al Segpay Merchant Portal**. Reporte mensual de contenido flaggeado, removals y takedowns, enviado al portal del PSP. Cuando no hay actividad relevante en el mes, se envía nil report explícito antes del día 5 del mes siguiente. Hoy: cero job, cero endpoint de export, cero procedimiento documentado. Datos crudos existen (`backoffice_access_audit_log`, `stream_moderation_events`, `stream_moderation_reviews`, `model_asset_reviews`). Bloqueante para cerrar onboarding Segpay. Estimación: sub-paquete dedicado.

5. **Age estimation del consumidor para USA estados con AVS + UK Online Safety Act**. El frente Didit cliente ya cubre la base europea (estimación facial primaria con Adaptive Age Verification). Para mercados USA con AVS activo y UK bajo OSA enero 2025, la exigencia regulatoria es más estricta y puede requerir step-up adicional o vendor específico. Trabajo posterior, no bloquea TEST. Bloqueante para activar geografías específicas.

6. **Vendor CSAM dedicado** (PhotoDNA / Thorn Safer o equivalente). ADR-030 línea 92 lista los candidatos sin priorizar. Hoy CSAM cubierto exclusivamente por Sightengine `minor` (sintetizado desde `faces[*].attributes.age.minor` -> CRITICAL kill switch en código) + layered defense (KYC modelo APPROVED requerido para emitir + KYC cliente APPROVED + ToS firmado). Reabrir si el card brand endurece exigencias o si MINORS de Sightengine muestra falsos negativos en operación. **Prioridad sube a alta pre-go-live PROD** si volumen lo justifica. Trabajo: contrato + integración técnica + hash database access (NCMEC/IWF típico).

7. **Activación SIGHTENGINE en AUDIT y PROD**. Sub-paquetes dedicados independientes. Replicarán bucket S3 + Lifecycle + Public Access Block + SSE-S3, IAM policy inline en `sharemechat-ec2-{audit,prod}-role`, credenciales `MODERATION_SIGHTENGINE_*` con esquema dual `config.env` / `secrets.env`, switch operativo `active_mode` vía endpoint admin. PROD adicionalmente requiere salto del plan Free al Starter $29/mes del vendor antes de la activación (ver `psp-strategy.md` para coste operativo). **AUDIT no es bloqueante**; **PROD es bloqueante para go-live público del 1-jul-2026 con moderación real** — sin él PROD seguiría con MOCK GREEN siempre, lo que es indefendible ante el card brand.

**Naturaleza del bloque**: estas 7 deudas son catálogo, no orden impuesto. El operador secuencia caso por caso. Las deudas tácticas del frente moderación (métricas de latencia, panel admin preview de evidencia, separación de pools S3, centralización de @Async, saneamiento de WARN del mapper, runbook permanente del setup entornal) viven en la entrada de bitácora del 2026-06-27 (`project-log.md`), no aquí.
