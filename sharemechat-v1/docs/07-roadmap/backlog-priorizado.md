...
| **EXIF/metadatos: strip en subida de fotos** | 🟢 en TEST (2026-08-29) · PROD gateado a OWASP | Strip **lossless** del EXIF/GPS del JPEG en `StorageService.store()` vía `ImageMetadataScrubber` (Apache Commons Imaging). Cubre todas las imágenes que pasan por storage. **PNG/WEBP/GIF sin strip** por ahora (gap: JPEG cubre ~99% del riesgo GPS). Falta: correr OWASP dependency-check con NVD key antes de PROD (no pudo correr en el entorno). | `ImageMetadataScrubber`; `S3/LocalStorageService.store()`; test verde |
| **Age-gate invitado: rate-limit anti-abuso** | 🟢 en TEST (2026-08-29) | **Reencuadre honesto:** el age-gate es un "confirmo 18+" autodeclarado + registro de consentimiento (auditoría), **NO** una barrera de contenido — el control de edad real del contenido es **Didit** (gate en matching+pago). Un "clic 18+" no se puede hacer a prueba de bots (httpOnly no frena a un cliente HTTP crudo). Hecho: **rate-limit por IP** de `/api/consent/age-gate` y `/terms` (reusa `ApiRateLimitService`, `consent:ip`, 30/5min) → frena creación masiva de registros. | `ConsentController`; `ApiRateLimitService.checkConsentIp` |
