...
|---|---|---|---|
| **Frente de TESTS + CI (metodología)** | 🟢 backend blindado (2026-08-14) | CI (`.github/workflows/ci.yml`, `on: push`) **verde**, backend (`mvnw test`) + frontend en cada push. **98 tests** de integración/unit (`@SpringBootTest`+Testcontainers MySQL 8.4 sobre baseline determinista) blindando: **dinero** (TransactionService incl. payout), **streaming** (endSession), **matching** (unit), **trial**, **tramos** (mapeo+Pro+tarifa), **Soporte** (bot IA 3 capas + humano claim/release/resolve), **Master COMPLETO** (splits+invitaciones+payout+suspensión), **KYC** (mapeo vendor→estado + webhooks Veriff/Didit con firma HMAC e idempotencia) y **OAuth account-linking**. Fases 0-3 del roll-out ADR-059 + periferia sensible. **Pendiente: login federado `AuthGoogleController` (vía slice MockMvc, decidido 2026-08-14) + Fase 4 (frontend: 0 tests → Jest/RTL + E2E Playwright, el grueso restante).** | ADR-059; `src/test/java/com/sharemechat/**`; `docs/project-log.md` 2026-08-14 |
