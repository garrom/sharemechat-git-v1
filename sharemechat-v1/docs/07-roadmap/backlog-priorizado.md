...
| **EXIF/metadatos: strip en subida de fotos** | 🔴 no implementado | Fotos personales de modelos y clientes se suben a S3 **tal cual** (`ModelAssetService.upload` sube el `MultipartFile` sin re-encode) → EXIF/GPS conservado = riesgo de doxxing de ubicación. Falta el strip estándar del sector (GPS + metadatos) en el pipeline de subida. | `ModelAssetService.upload`; sin EXIF-strip en el repo |
