...
| **Compliance: 5 políticas PSP** | 🟡 textos HECHOS 2026-08-31 · firma legal pendiente | **Las 5 producidas como documento propio** (verificado contra código): Content Management, Consumer Age Verification (nueva 2026-08-31), Complaint & Content Removal (Complaints+Appeals), Model Agreement, Chargeback-Fraud Mitigation (nueva 2026-08-31). Las 12 salen de `ops/legal-pdfs/generate_legal_pdfs.py`. Bloqueante restante: **(a) firma/revisión legal externa** (a criterio del operador) + **(b) backing técnico** (canal de quejas público + SLA 5d + reporting mensual PSP → frentes P2 moderación/DSA). Pendiente publicar los 2 PDFs nuevos a `s3://assets-sharemechat-*/legal/`. | `generate_legal_pdfs.py`; `compliance-deliverables.md` §2 |
| **Página pública de modelo `/m/:slug`** | ❌ DESCARTADA 2026-08-31 ([ADR-063](../06-decisions/adr-063-modelo-random-match-descarta-pagina-publica-modelo.md)) | No se implementa. El producto es match aleatorio 1-a-1 tipo CooMeet (sin escaparate por modelo); `/m/:slug` era patrón de cam site. La afiliación que la sostenía ya cayó (ADR-052). ADR-048 y `model-profile-strategy.md` quedan SUPERSEDED. **Hueco que deja: la captación de modelos se queda sin palanca** → nuevo frente de **reclutamiento directo/B2B** (pendiente de abrir). | ADR-063 supersede ADR-048 |
