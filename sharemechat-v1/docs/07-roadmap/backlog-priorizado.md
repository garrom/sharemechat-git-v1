...
| **Age-gate invitado: bypass por `consent_id` arbitrario** | 🔴 abierto (seguridad) | El `consent_id` lo genera el cliente y el backend acepta cualquiera → un cliente automatizado se salta la fricción 18+ enviando cualquier UUID + `POST /api/consent/age-gate`. Control de compliance (no barrera de auth), pero superficie a cerrar. | `ConsentController`; `ConsentService.hasGuestAgeGate` |
