...
| Tarea | Estado (código) | Qué es | Evidencia |
|---|---|---|---|
| **Backfill de tests — gaps de cobertura (auditoría 2026-08-30)** | 🟡 en curso (Lote 1 hecho) | Auditoría exhaustiva (5 revisores) tras el crecimiento del código: el núcleo de riesgo (dinero de salida/webhooks, gates de sesión/permiso, moderación cámara, matching backend) está cubierto y la disciplina "lo nuevo con test" se mantuvo, pero hay gaps reales sin red. **Orden**: (1) seguridad/compliance — `StorageController.canReadFile` (**IDOR** de docs KYC), `GdprExportService` (export art.15 que vacía datos en silencio), `ModelContract(Manifest)Service` ✅Lote1, `AuthController` (rotación/reuso refresh), `ModerationReportService` (suspend+revoca sesión), `PasswordResetService`; (2) dinero de ENTRADA — `PspOrchestratorService.createCheckout`, `PayoutMethodService`, `PspProviderConfigService` ✅Lote1, `store()` ✅Lote1; (3) frontend `realtime/` (WebRTC/matching, cero tests; empezar por funciones puras `iceObserver`/`webrtcConfig`) + E2E streaming/onboarding modelo; (4) MockMvc de controllers finos. | tests bajo `src/test/**`; auditoría bitácora 2026-08-30 |
