...
|---|---|---|---|
| **Frente de TESTS + CI (metodología)** | 🟢 backend cerrado + unit/component robusta + E2E happy-paths críticos cubiertos (2026-08-15) | CI (`.github/workflows/ci.yml`, `on: push`) **verde**, **3 jobs**: backend (`mvnw test`) + frontend (`react-scripts test`) + **e2e (Playwright/Chromium, backend mockeado)** en cada push. **404 tests unit/component + 4 specs E2E**: **backend 137** (blindado — dinero/payout, streaming, matching, trial, tramos+Pro+tarifa, Soporte bot+humano, Master COMPLETO, KYC mapeo+webhooks HMAC, Auth OAuth-linking **+ login federado Google**, **+ gate Product Operational Mode modo-por-rol ADR-009** (service + filter REST + WS interceptor, `resolveIsModel` fail-closed vía helper compartido `UserRoleUtils.isModelOrCandidate` — DRY, sin divergencia entre las 3 copias), **+ webhook PSP `PspWebhookOrchestrator`** [tolerancia parciales cripto 3%/0,50/5 + crédito BFPM: finished/partial-ok/partial-ko/sin-cripto, integración]) **+ frontend 267** (con **`config/http.js` `apiFetch`** [mantenimiento 5xx vs gate PRELAUNCH, refresh transparente 401→retry, gates EMAIL_NOT_VERIFIED/CLIENT_KYC_REQUIRED, skip-refresh /auth] **+ `SessionProvider`** [rutas públicas vs loadMe /users/me, 401→auth:logout, refresh, updateUiLocale con/sin sesión]) (Jest/RTL — registros cliente/modelo/master, `SessionHUD` timer/coste, checkout polling, hooks/dominio [traductor, polling, interacción, `useAppModals`, settings, pending-count, `useSupportChat` chat bot+humano, `useFrameCapture` moderación cámara]; **utils puros** [`attribution`, `runtimeEnv`, `normalizeNickname`, `registerErrorMessage`, `runtimeSurface` routing, `backofficeAccess` permisos, `clientKycGate` gate pago, `mediaState`, `virtualCameraGuard` anti-fraude cámara, `apiErrors`, `emojiUtils`]; **componentes seguridad/infra/UX** [`RequireRole` gate PRELAUNCH ADR-009, `MaintenanceProvider` failover, `CookieBanner` consent GDPR, `TrialCooldownModal`, `LocaleSwitcher` ADR-022/025, `ModalProvider`]). **E2E** (`frontend/e2e/`, backend mockeado): `smoke` + `registro-cliente` (age-gate→género→cliente→form→éxito) + `login` (credenciales→`refresh`→redirige a `/client`) + `checkout-primer-pago` (FORM_CLIENT: gates KYC+email→modal de pack→redirige a la `invoiceUrl` del PSP). Los 4 happy-paths críticos cubiertos. Capa unit/component/integración/E2E **ROBUSTA**. Deuda menor pendiente: clase base común de los `IntegrationTest`. Nota: el E2E cazó el día 1 un bug de empaquetado (`process`/`buffer` sin declarar → `npm ci` limpio rompía el bundle, página en blanco; ya corregido). | ADR-059; `src/test/java/**` + `frontend/src/**/*.test.*` + `frontend/e2e/**`; `docs/project-log.md` 2026-08-15 |
