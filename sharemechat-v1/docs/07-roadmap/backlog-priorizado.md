...
|---|---|---|---|
| **Frente de TESTS + CI (metodología)** | 🟢 backend cerrado + Fase 4 en marcha (2026-08-15) | CI (`.github/workflows/ci.yml`, `on: push`) **verde**, backend (`mvnw test`) + frontend (`react-scripts test`) en cada push. **165 tests**: **backend 107** (blindado — dinero/payout, streaming, matching, trial, tramos+Pro+tarifa, Soporte bot+humano, Master COMPLETO, KYC mapeo+webhooks HMAC, Auth OAuth-linking **+ login federado Google**) **+ frontend 58** (Fase 4 Jest/RTL — registros cliente/modelo/master, `SessionHUD` timer/coste, checkout polling, y hooks/dominio: traductor, polling [fake timers], interacción core + helpers dominio, `useAppModals`). Fases 0-3 + periferia backend **CERRADAS**; **Fase 4 frontend EN MARCHA**. Pendiente: más hooks/utils frontend + **E2E (Playwright)**; deuda menor (clase base común de los `IntegrationTest`). | ADR-059; `src/test/java/**` + `frontend/src/**/*.test.*`; `docs/project-log.md` 2026-08-15 |
