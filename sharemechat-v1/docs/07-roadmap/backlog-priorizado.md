...
|---|---|---|---|
| **Frente de TESTS + CI (metodología)** | 🟢 en curso (2026-08-14) | CI (`.github/workflows/ci.yml`, `on: push`) **verde**, backend (`mvnw test`) + frontend en cada push. ~59 tests de integración/unit (`@SpringBootTest`+Testcontainers MySQL 8.4 sobre baseline determinista) cubriendo el core antes sin red: **dinero** (TransactionService incl. payout), **streaming** (endSession), **matching**, **trial**, **tramos** (mapeo+Pro+recorte tarifa), **bot IA soporte** (heurística+rate-limit+orquestación con Claude mockeado) y **Master** (splits `setInternalShare` + invitaciones `inviteModel`). Fases 0-3 del roll-out ADR-059 materializadas. Pendiente: `MasterPayoutService`, KYC/Google Sign-In, soporte humano, y frontend/E2E (Fase 4). | ADR-059; `src/test/java/com/sharemechat/{integration,handler,support,master}`; `docs/project-log.md` 2026-08-14 |
