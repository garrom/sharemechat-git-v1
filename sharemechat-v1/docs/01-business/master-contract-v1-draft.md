...
5.1. **Porcentaje sobre ingreso bruto**: el importe generado por cada stream de las modelos bajo umbrella del Master se calcula individualmente para cada modelo según el tramo (T1, T2, T3, T4) que le corresponda en la tabla vigente `model_pricing_tiers` publicada por SharemeChat en el **Dashboard Master** del titular (sección "Régimen económico") y **consultable en todo momento tras autenticación**. El tramo de cada modelo depende de su propia facturación bruta rolling 30 días. El importe resultante para cada stream se acredita en la cuenta del Master, quien recibe así la suma de lo devengado por todas sus modelos.

La tabla vigente es la fuente única de referencia; el Master declara haberla consultado antes de aceptar el presente contrato. SharemeChat se reserva el derecho a modificar los umbrales y porcentajes de dicha tabla con preaviso mínimo de 30 días naturales, mediante notificación al Master vía email al correo de contacto registrado; la modificación no requiere nueva versión del presente contrato salvo que también altere las cláusulas 5.2–5.5 u otras condiciones sustantivas.
