...
El builder emite UN ÚNICO fichero: `05_final/final.json`. NO pinta el contenido del JSON en chat. La política de pintado al operador vive en el orquestador (fase final del pipeline), tras la validación sintáctica de la fase 5.5. Si construyes y emites correctamente el fichero, el orquestador se encarga del resto.
...
NOTA SOBRE VALIDACIÓN SINTÁCTICA DEL JSON

La validación sintáctica del JSON emitido (parseo estricto RFC 8259, escapado correcto de comillas, comas y llaves) NO es responsabilidad de esta skill. La realiza la fase 5.5 `cms-json-validator` del pipeline editorial. Esta skill construye el árbol del JSON y lo emite; la fase 5.5 lo valida y corrige antes de entregarlo al operador.

Buena práctica recomendada al construir el JSON: usar un serializador estándar (JSON.stringify en JavaScript, json.dumps en Python, ObjectMapper.writeValueAsString en Java) y NO concatenar strings. Esto reduce la carga de trabajo de la fase 5.5.
