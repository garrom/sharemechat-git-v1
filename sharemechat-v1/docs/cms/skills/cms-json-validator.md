...
ALCANCE Y LÍMITES (ADR-045 SUBPASADA 2B)

Esta skill valida SINTAXIS del JSON y realiza UNA validación estructural ligera de keywords per-locale (ver "Check estructural de target_keywords" abajo). NO valida coincidencia entre la primary del operador y la primary emitida por el pipeline.

El check de coincidencia primary operador vs output se aplica en el BACKEND en `ContentRunService.checkPrimaryMismatch` al recibir el JSON vía `POST /admin/content/articles/{id}/runs/{runId}/apply-bilingual`. Si detecta mismatch, el backend responde con `runStatus=REJECTED` y un `ValidationErrorDTO` accionable del tipo `"IA propuso primary keyword 'X' pero el operador declaró 'Y' en locale ES/EN. El pipeline debe honrar la keyword del operador."`.

Esta skill se limita a validar cardinalidad estructural porque la fuente autoritativa del input operador vive en la BD del CMS, no en el JSON que el validator recibe; duplicar esa validación aquí generaría falsos positivos/negativos sin ganar defensa real. Si un futuro operador se pregunta "por qué la skill no cazó este mismatch", la respuesta está aquí: por diseño; el backend lo hace mejor con acceso a BD.
...
- En caso de éxito (algún intento parsea limpio Y el check estructural de target_keywords pasa): sobrescribes `05_final/final.json` con la versión corregida y emites `05_final/validator_report.md` con el resumen del trabajo.
- En caso de fallo (3 intentos de parseo agotados, o check estructural de target_keywords falla): NO sobrescribes `05_final/final.json`. Lo renombras a `05_final/final.broken.json` para que el orquestador y el operador detecten inmediatamente que el output canónico falta. Emites `05_final/validator_report.md` con el diagnóstico completo.
...
- Si `05_final/final.json` NO EXISTE pero existe `05_final/final.broken.json` → la fase 5.5 abortó tras 3 intentos o el check estructural falló. Leer `validator_report.md` para diagnóstico antes de tocar nada.
...
- locales.{es,en}.target_keywords[].term (términos con espacios o apóstrofes; ADR-045)
...
CHECK ESTRUCTURAL DE target_keywords (ADR-045)

Tras el parseo limpio, verifica en cada `locales.{es,en}.target_keywords`:
- Es un array (no null, no objeto).
- EXACTAMENTE 1 objeto con `type == "primary"`.
- 0..5 objetos con `type == "secondary"`.
- Cada objeto tiene `term` string no vacío.

Si alguna condición falla, el JSON NO es apto para el CMS aunque parsee limpio. Trata como fallo estructural: NO sobrescribas `final.json`, renómbralo a `final.broken.json`, y emite el diagnóstico específico en `validator_report.md` con el path exacto (p. ej. `locales.en.target_keywords: 2 objetos type=primary detectados`).

Este check NO valida coincidencia primary operador vs output (ver "ALCANCE Y LÍMITES" arriba). Solo cardinalidad estructural.

POLÍTICA DE ÉXITO

Cuando algún intento (1, 2 o 3) parsea limpio Y el check estructural de target_keywords pasa:
...
2. Emite `05_final/validator_report.md` con:
   ...
   - Resultado del check estructural target_keywords: "OK" o detalle si hubo warning no bloqueante (no debería haber; los fallos son bloqueantes).
   - Confirmación de cierre: "Este JSON debería pasar el gate cliente-side del admin (JSON.parse limpio). El check semántico de coincidencia primary operador vs output lo hará el backend (ContentRunService.checkPrimaryMismatch) al recibir el JSON."

POLÍTICA DE FALLO TRAS 3 INTENTOS O FALLO ESTRUCTURAL DE KEYWORDS

Cuando los 3 intentos fallan, o cuando el check estructural de target_keywords falla:
...
3. Emitir `05_final/validator_report.md` con:
   - Motivo: "3 intentos de parseo agotados" o "check estructural target_keywords falló".
   - Si es fallo de parseo: listado breve de los 3 intentos, qué errores se detectaron, qué fixes se intentaron, por qué falló al re-parsear. Posición exacta del último error sin resolver: línea, columna, fragmento de ~40 caracteres alrededor del char offset con un caret (`▸` o equivalente) apuntando al carácter problemático.
   - Si es fallo estructural: path del campo afectado y cardinalidad detectada vs esperada.
   - Mensaje explícito de cierre: "VALIDATOR ABORTÓ. final.json renombrado a final.broken.json. NO importar al CMS hasta corregir a mano."
...
El validator sobrescribe el fichero `05_final/final.json` (si éxito) o lo renombra a `05_final/final.broken.json` (si fallo). En ambos casos, NO pinta el contenido del JSON en chat. El pintado del JSON validado al operador lo hace el orquestador en su reporte final, no esta skill.
...
- NO valida coincidencia primary operador vs output. Ese check vive en el backend (ADR-045; ver "ALCANCE Y LÍMITES" arriba). Esta skill se limita al check estructural de cardinalidad target_keywords.
...
- Resultado: éxito (intento 1, 2 o 3) | fallo tras 3 intentos | fallo estructural target_keywords.
- Si éxito: nº de intentos usados, nº de fixes aplicados (0 si parseó al primer intento sin tocar nada), check estructural target_keywords OK.
- Si fallo: posición del último error sin resolver (línea, columna) o path del campo con fallo estructural; sugerencia de qué campo inspeccionar en `04_review/` o en la construcción de fase 5.
