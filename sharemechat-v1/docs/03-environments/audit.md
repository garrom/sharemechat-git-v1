...
- `CustomErrorResponses` vacío (`Quantity=0`). El SPA-fallback queda gestionado por la CloudFront Function `redirect-spa-audit` asociada al `viewer-request` del `DefaultCacheBehavior`, sin enmascaramiento de 403/404 del backend. Detalle en [incident-notes.md](docs/_archive/04-operations/incident-notes.md) sección "Fix CloudFront AUDIT 2026-05-21 (paquete 10.A.2)".
