...
Sustituye y generaliza al diseño previo de "gating de prelaunch" como flag binaria, que queda absorbido por esta decisión.
...
- **TEST/AUDIT/PRO sin PSP real habilitado para simulación directa**: los endpoints directos de acreditación económica pueden cerrarse server-side sin cerrar payout ni webhooks.
...
- cómo modelar registro de cliente, registro de modelo y simulación económica directa de forma independiente del modo, para poder cerrarlos en TEST/AUDIT manteniendo el producto abierto
...
Se introduce una capa nueva, **Product Operational Mode**, gobernada por una propiedad principal, dos flags de registro independientes y una flag de simulación económica directa, aplicada server-side mediante un filtro REST y un interceptor WebSocket.
...
### Flag de simulación económica directa

Una flag independiente del modo gobierna los endpoints directos de simulación económica:

- propiedad: `product.simulation.transactions-direct.enabled`
- variable: `PRODUCT_SIMULATION_TRANSACTIONS_DIRECT_ENABLED`
- default seguro: `false`

Cuando está a `false`, bloquea exclusivamente:
...
- `POST /api/billing/ccbill/notify`
- otros webhooks externos

La flag aplica también con modo `OPEN`. La allowlist por `userId` no la salta, porque no es una excepción de admisión de producto sino un cierre explícito de simulación económica directa por entorno.
...
`POST /api/billing/ccbill/session` no es whitelist permanente: queda clasificado como path de producto y se bloquea en modos restrictivos.
...
- `SIMULATION_DISABLED` para endpoints económicos directos cerrados, con `scope` `transactions-direct`
...
- PRO puede mantenerse en `OPEN` sin riesgo de endpoints directos de simulación si `product.simulation.transactions-direct.enabled=false`
- TEST puede abrir simulación directa de forma explícita para trabajo interno
- AUDIT queda cerrado por defecto para simulación directa, evitando contaminación de datos de revisión externa
...
- **La flag de simulación no sustituye PSP**: no reemplaza la integración CCBill real ni la verificación de firma del webhook.
- **Flujo directo cerrado por diseño**: si CCBill no está plenamente operativo y la flag queda `false`, el flujo directo de promoción/acreditación por endpoint directo queda cerrado. Es correcto para AUDIT/PRO; TEST puede abrirlo explícitamente.
- **Reapertura accidental en PRO**: activar `PRODUCT_SIMULATION_TRANSACTIONS_DIRECT_ENABLED=true` en PRO sería riesgo crítico operativo. Mitigación: default `false`, checklist de entorno y revisión antes de dinero real.
...
- cierre de simulación económica directa por entorno sin afectar payout ni webhooks
...
- implementación backend completada en los tres componentes definidos: `ProductOperationalModeService` (lógica central), `ProductOperationalModeFilter` (REST) y `ProductOperationalModeWsInterceptor` (handshake WebSocket). Registrados en `SecurityConfig` y `WebSocketConfig`. Propiedades versionadas en `application.properties` y `application-audit.properties` con defaults seguros.
- validación operativa actualmente cubierta:
  - cierre de registro de cliente (`POST /api/users/register/client`) en TEST y AUDIT con `PRODUCT_REGISTRATION_CLIENT_ENABLED=false`
  - cierre de registro de modelo (`POST /api/users/register/model`) en TEST y AUDIT con `PRODUCT_REGISTRATION_MODEL_ENABLED=false`
  - `PRODUCT_SIMULATION_TRANSACTIONS_DIRECT_ENABLED` implementada con default seguro `false`
  - bloqueo de `POST /api/transactions/first` y `POST /api/transactions/add-balance` validado en TEST con 503 `SIMULATION_DISABLED`
  - `POST /api/transactions/payout` validado como no afectado por la flag de simulación directa
  - `POST /api/billing/ccbill/session` añadido como path de producto para quedar bloqueado por modos restrictivos
  - ausencia de regresión sobre login, matching, sesiones realtime y gifts con `PRODUCT_ACCESS_MODE=OPEN`
  - logs `[PRODUCT-MODE]` emitidos en ambos entornos con los códigos esperados
  - ver detalle en `docs/04-operations/runbooks.md` (sección "Runbook de Product Operational Mode")
- validación pendiente:
  - modos `PRELAUNCH`, `MAINTENANCE` y `CLOSED` aplicados a producto (login, refresh, endpoints REST funcionales y handshake WS)
  - frontend: tratamiento de los códigos `PRODUCT_UNAVAILABLE`, `PRODUCT_MAINTENANCE`, `REGISTRATION_CLOSED` y `SIMULATION_DISABLED` donde aplique
  - allowlist por `userId`: implementada en backend como mecanismo de excepción dentro de modos restrictivos; no se ha ejercitado en entornos porque depende de que un modo restrictivo esté activo
- la presente ADR es la fuente de verdad de la decisión; la operativa concreta (configuración, comandos, validación) vive en `docs/04-operations/runbooks.md`
