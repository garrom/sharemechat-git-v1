...
8. **Fase 7: Activación PROD + refinamientos por UX real (2026-07-18)**. **HECHA**. Retirado el override belt-and-suspenders del `application-prod.properties` (`psp.nowpayments.enabled=false` fuerza flip → property base `PSP_NOWPAYMENTS_ENABLED=true` gobierna). Deploy backend PROD con credenciales reales NOWPayments. Primer flujo real end-to-end validado por operador (transferencia ING → Kraken → compra USDT → hosted checkout PROD → webhook → BFPM aplicado). **Refinamiento nuevo — filtro `pay_currencies` por pack** en `PspOrchestratorService.allowedPayCurrenciesForPack(packKey)` + `CreateInvoiceRequest.payCurrencies` + `NowPaymentsHttpClient` pone `pay_currencies` array en el body de `POST /v1/invoice`. Motivación: sandbox 2026-07-17 mostró que BTC en P10 (10 EUR) da `"Crypto amount 0.00017823 is less than minimal"` porque el mínimo NOWPayments para BTC ronda 12-30 EUR según cambio. El hosted checkout mostraba el error dentro del selector y bloqueaba Next Step, mala UX. Tabla por pack: P10 → `[usdttrc20, usdterc20, usdcerc20]` (sin BTC); P20 y P40 → las 4 (BTC + 3 stablecoins). Si el operador añade/quita monedas en el panel del vendor, la lista Java debe reflejarlo para coherencia. Commit `ed03a3e`.

Aproximadamente 8 subpasadas efectivas (las 8 hechas al 2026-07-19).
