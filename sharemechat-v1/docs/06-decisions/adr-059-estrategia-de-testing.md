...
- **Cobertura (98 tests de integración/unit)**: dinero (`TransactionService`: primer pago, packs+bonus, gift individual y bajo Master, refunds, payout×4), streaming (`StreamService.endSession`), matching (unit Mockito sobre `MatchingHandlerSupport`: pairing, ranking por idioma, `next`, trial), trial (`UserTrialService`), tramos (`ModelTierService`: mapeo umbral→tramo, snapshot, Estatus Pro, recorte de tarifa), **Soporte** (bot IA: heurística de ticket unit + rate-limit + orquestación `SupportBotService` con `ClaudeApiClient` mockeado; y humano `SupportHumanHandlingService`: claim/release/resolve/sendHumanMessage), **Master completo** (splits `setInternalShare`, invitaciones `inviteModel`, payout `MasterPayoutService.requestPayout`, suspensión `MasterSuspensionService` suspend/reactivate), **KYC** (mapeo vendor→estado interno unit `mapInternalStatusFromCode`/`mapInternalStatusFromDiditStatus` + webhooks `processVeriffWebhook`/`processDiditWebhook` integración con firma HMAC e idempotencia), y **OAuth account-linking** (`OAuthAccountController` link/unlink/initial-password, unit Mockito).
...
Pendiente (backend, poco): **login federado** (`AuthGoogleController.googleAuth` — lógica en un controller gordo de 12 deps + pipeline age-gate/rate-limit/country/risk; se abordará vía **slice MockMvc con `@MockBean`**, la opción menos invasiva —sin refactor de producción— decidida por el operador el 2026-08-14); casos menores de matching (`matchClient` sin-oferta) y age estimation Didit CLIENT; deuda menor (extraer clase base común de los `IntegrationTest`). Pendiente (grueso): **Fase 4** — el frontend hoy tiene **0 tests**; primeros Jest/RTL de componentes (registro, pago, `SessionHUD`) + primeros E2E (Playwright). Extracción de seams (puertos/policies) según se endurezca el core.
