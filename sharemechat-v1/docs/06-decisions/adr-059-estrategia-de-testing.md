...
- **Verificación local**: los tests Testcontainers NO corren en local (Windows: docker-java no conecta al named pipe de Docker Desktop) → **la CI Linux es el juez**; los unit puros (matching, heurística) sí corren en local. **Runbook operativo completo** (qué corre sin Docker, por qué Testcontainers no conecta, fix si Docker no arranca) en [`docs/03-environments/test.md` §*Docker Desktop y Testcontainers en local*](../03-environments/test.md#docker-desktop-y-testcontainers-en-local).
...
Segunda tanda de tests frontend el mismo día, atacando el hueco de **utils puros críticos y componentes de seguridad/infra** que estaban sin cubrir. Frontend Jest **91 → 246** (backend 107 + frontend 246 = **≈353** total unit/component) + 4 specs E2E. Encadenado por batches (rama → verde local → CI 3 jobs → merge verificado):
...
- **Hooks grandes** — `useSupportChat` (chat soporte bot+humano: optimistic, rate-limit, escalado, refetch al entrar en HUMAN_HANDLING, modo pinned de ticket ADR-054; harness-captura-API + mock `supportApi`) y `useFrameCapture` (captura de frames para moderación IA ADR-036: multipart, cadencia con fake timers, política de errores 4xx-detiene/5xx-reintenta; mock de `document.createElement` video/canvas + `apiFetch`).
