#!/usr/bin/env python3
"""
PROD access blocker (DRY-RUN).

Consume el summary diario del clasificador y produce, sin tocar nginx:

  - <OUTPUT_ROOT>/YYYY-MM-DD.deny-prod-ips.proposed.conf
...
Este componente NO escribe /etc/nginx/deny-prod-ips.conf, NO ejecuta
nginx -t ni reload y NO bloquea trafico real. El bloqueo real se activa
en una fase posterior con un cambio explicito controlado por operacion.
...
def run_blocker(args: argparse.Namespace) -> int:
    ...
    # Inyectar rangos CloudFront origin-facing como allowlist CIDR (fix
    # perimetro 2026-06-07): garantiza que el blocker nunca propondra ni
    # aplicara deny sobre una IP de un edge CloudFront aunque el
    # clasificador la marque por confusion.
    cf_cidr_count = merge_cloudfront_cidrs(allowlist)
    if cf_cidr_count:
        print(f"loaded {cf_cidr_count} CloudFront origin-facing CIDRs into allowlist")
    ...
    proposed_path = output_root / f"{day}.deny-prod-ips.proposed.conf"
...
def resolve_summary_input(
    ...
    if not path.exists():
        ...
        # tras rearrancar la EC2 con timers en cola por Persistent=true y el
...
# ---------------------------------------------------------------------------
# CloudFront origin-facing guard (fix perimetro 2026-06-07)
# ---------------------------------------------------------------------------
# Aunque el clasificador puede sugerir banear una IP que en realidad es un
# edge CloudFront (en caso de fallo del real_ip de nginx), este guard
# garantiza que el blocker NUNCA escribira un deny sobre una IP que caiga
# en los rangos CLOUDFRONT_ORIGIN_FACING. Los CIDRs se cargan del mismo
# fichero que usa nginx en su directiva set_real_ip_from (fuente unica
# de verdad). Se inyectan en la allowlist como CIDR para que la logica
# existente los descarte como skip_allowlisted.

CLOUDFRONT_CIDR_FILE_DEFAULT = "/etc/nginx/cloudfront-origin-facing.conf"


def merge_cloudfront_cidrs(allowlist: "Allowlist", path: Optional[str] = None) -> int:
    """Parsea CIDRs de set_real_ip_from <CIDR>; en el fichero indicado y
    los añade a la allowlist como redes CIDR con comentario 'cloudfront_origin_facing'.
    Devuelve el numero de CIDRs cargados. Si el fichero no existe o esta
    vacio, devuelve 0 y no falla."""
    target = Path(path or CLOUDFRONT_CIDR_FILE_DEFAULT)
    if not target.exists():
        return 0
    pattern = re.compile(r"^\s*set_real_ip_from\s+([^;]+);")
    count = 0
    for line in target.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
        allowlist.networks.append(net)
        allowlist.comments[str(net)] = "cloudfront_origin_facing"
        count += 1
    return count
...
def apply_carril_a_to_nginx(
    ...
    try:
        ...
        content_parts.append(
            f"# Auto-generado por prod-access-blocker - Carril A - {day}"
        )
        if carril_a_new_ips:
...
def write_proposed_deny_list(path: Path, decisions: List[Decision], day: str) -> None:
    lines: List[str] = []
    lines.append("# Proposed deny list for PROD nginx (DRY-RUN).")
    lines.append(f"# Generated: {day}")
    lines.append("# This file is advisory only. It is NOT loaded by nginx.")
    lines.append("# Active deny list in EC2: /etc/nginx/deny-prod-ips.conf")
    lines.append("#")
...
def write_diff_report(
    ...
    lines: List[str] = []
    lines.append(f"PROD access blocker diff - {day} ({mode_label})")
    lines.append("")
