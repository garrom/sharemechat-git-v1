...
WORK_ROOT="${WORK_ROOT:-/var/lib/sharemechat-prod-access-normalizer}"
STATE_ROOT="${STATE_ROOT:-${WORK_ROOT}/state}"
TMP_ROOT="${TMP_ROOT:-${WORK_ROOT}/tmp}"
CHUNK_ROOT="${CHUNK_ROOT:-${WORK_ROOT}/chunks}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/var/log/sharemechat-prod-access-normalizer}"
LOCK_FILE="${LOCK_FILE:-${WORK_ROOT}/normalize.lock}"
NGINX_ACCESS_LOG="${NGINX_ACCESS_LOG:-/var/log/nginx/access.log}"
CF_BUCKET="${CF_BUCKET:-sharemechat-cf-logs-prod}"
CF_PREFIX_PRODUCT="${CF_PREFIX_PRODUCT:-cloudfront/prod/}"
CF_PREFIX_ADMIN="${CF_PREFIX_ADMIN:-cloudfront/admin-prod/}"
