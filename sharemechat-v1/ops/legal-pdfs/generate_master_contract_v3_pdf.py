# -*- coding: utf-8 -*-
"""
Generador PDF del borrador Contrato Master v3 (docs/01-business/
master-contract-v3-draft.md). Look and feel coherente con
generate_legal_pdfs.py.

Uso:
    python ops/legal-pdfs/generate_master_contract_v3_pdf.py
Output:
    .tmp/master-contract-v3-draft.pdf
"""
...
def _parse_markdown_to_flowables(md_text):
    ...
    while i < n:
        ...
        if line.startswith('## '):
            ...
            continue
        if line.startswith('### '):
            flowables.append(Paragraph('<b>' + _inline_md_to_rl(line[4:].strip()) + '</b>', style_body))
            i += 1
            continue
        if line.strip() == '---':
...
def main():
    ...
    md_path = os.path.join(repo_root, 'docs', '01-business', 'master-contract-v3-draft.md')
    out_dir = os.path.abspath(os.path.join(repo_root, '..', '.tmp'))
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, 'master-contract-v3-draft.pdf')
    ...
    doc = SimpleDocTemplate(
        ...
        title='SharemeChat — Contrato Master v3.1 (borrador)',
        author='Shareme Technologies OÜ',
        subject='Contrato Master v3.1 borrador',
