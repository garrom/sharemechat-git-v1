...
Consume el summary diario del clasificador y produce, sin tocar nginx:
  ...
  - <OUTPUT_ROOT>/YYYY-MM-DD.deny-audit-ips.proposed.conf
...
Este componente NO escribe /etc/nginx/deny-audit-ips.conf, NO ejecuta
...
def run_blocker(args: argparse.Namespace) -> int:
    ...
    proposed_path = output_root / f"{day}.deny-audit-ips.proposed.conf"
...
def apply_carril_a_to_nginx(
    ...
    try:
        ...
        content_parts.append(
            f"# Auto-generado por audit-access-blocker - Carril A - {day}"
        )
        if carril_a_new_ips:
...
def write_proposed_deny_list(path: Path, decisions: List[Decision], day: str) -> None:
    lines: List[str] = []
    lines.append("# Proposed deny list for AUDIT nginx (DRY-RUN).")
    lines.append(f"# Generated: {day}")
    lines.append("# This file is advisory only. It is NOT loaded by nginx.")
    lines.append("# Active deny list in EC2: /etc/nginx/deny-audit-ips.conf")
    lines.append("#")
...
def write_diff_report(
    ...
    lines: List[str] = []
    lines.append(f"AUDIT access blocker diff - {day} ({mode_label})")
    lines.append("")
