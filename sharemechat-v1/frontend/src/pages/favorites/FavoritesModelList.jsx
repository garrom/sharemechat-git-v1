...
function FavListItem({ user, avatarUrl, onSelect, onOpenMenu, selected = false, unread = 0, preview = '', lastAt = null, giftEmojiByName = null, menuOpen = false }) {
  ...
  // Bot: menú de acciones oculto entero.
    ...
    >
      <DotWrap>
        {isBot ? (
          ...
          <LetterAvatar $size={38} aria-hidden="true">{initialOf(displayName)}</LetterAvatar>
        )}
        <StatusDot
      ...
      {!isBot && (
        <FavMenuTrigger
          ...
          title={isMenuDisabled ? (blockedByOther ? i18n.t('favorites.menu.actionsUnavailable') : isBlocked ? (blockedByMe ? i18n.t('favorites.actions.unblock') : i18n.t('favorites.states.blocked')) : i18n.t('favorites.menu.unavailable')) : i18n.t('favorites.menu.moreOptions')}
...
export default function FavoritesModelList({ onSelect, reloadTrigger = 0, selectedId = null, autoSelectBot = false, onAutoSelectHandled = null, onItemsChange = null, gifts = [] }) {
