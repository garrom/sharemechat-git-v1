    ...
    <>
      ...
      <Wrapper onClick={handleBackdropClick}>
        <Dialog
          ...
          style={{width:'min(600px, calc(100% - 24px))',maxWidth:'none'}}
        >
          <Body data-kind="promo-video">
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8,gap:12}}>
              <div style={{display:'flex',alignItems:'center',gap:12}}>
                ...
                <div style={{display:'flex',flexDirection:'column'}}>
                  <span style={{fontSize:'0.78rem',color:'#9ca3af',textTransform:'uppercase',letterSpacing:'.08em'}}>
            ...
            <div style={{position:'relative',margin:'0 auto',width:'min(360px, calc(100% - 32px))',height:'calc(100vh - 220px)',borderRadius:14,overflow:'hidden',background:'#000'}}>
              {onAddFavorite && (
                <ButtonAddFavorite
                  ...
                  aria-label={i18n.t('common.actions.addToFavorites')}
                  title={i18n.t('common.actions.addToFavorites')}
              ...
              {canPrev && (
                <BtnPromoNav
                  type="button"
                  onClick={onPrev}
                  aria-label={i18n.t('modals.promoVideo.previous')}
                  title={i18n.t('modals.promoVideo.previous')}
              ...
              {canNext && (
                <BtnPromoNav
                  type="button"
                  onClick={onNext}
                  aria-label={i18n.t('modals.promoVideo.next')}
                  title={i18n.t('modals.promoVideo.next')}
            ...
            <div style={{marginTop:10,fontSize:13,opacity:0.9,textAlign:'left'}}>
              {video.durationSec != null && (
                <div>
                  <strong>{i18n.t('modals.promoVideo.duration')}</strong> {video.durationSec} s
                </div>
              )}
            </div>
          </Body>
